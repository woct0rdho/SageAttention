...
template <typename... OtherTensors>
void check_same_device(const Tensor& reference, const OtherTensors&... others) {
  const int32_t device_index = reference.get_device_index();
  STD_TORCH_CHECK(
      ((others.get_device_index() == device_index) && ...),
      "gfx12 native tensors must be on the same device");
}

struct GuardedHipStream {
  torch::stable::accelerator::DeviceGuard device_guard;
  hipStream_t stream;
};

GuardedHipStream current_hip_stream(const Tensor& tensor) {
  return {make_device_guard(tensor), get_current_cuda_stream(tensor)};
}
...
Tensor transpose_value_hnd_gfx12(Tensor value) {
  ...
  auto [device_guard, stream] = current_hip_stream(value);
...
Tensor transpose_value_fp8_scaled_hnd_gfx12(
  ...
  check_same_device(value, value_scale);
...
Tensor mean_nhd_gfx12(Tensor input) {
  ...
  auto [device_guard, stream] = current_hip_stream(input);
  if (use_short_mean) {
    dim3 block(1024);
...
std::vector<Tensor> prepare_qkv_hnd_smooth_f16_gfx12(
  ...
  check_same_device(query, key, value, key_mean);
  STD_TORCH_CHECK(query.dim() == 4 && key.dim() == 4 && value.dim() == 4,
...
#define SAGEATTN_DISPATCH_FUSED_Q_FP8_BC_HD_D64_CAUSAL(BC_, HD_) \
      if (block_rows == 64) { SAGEATTN_LAUNCH_FUSED_Q_FP8(BC_, HD_, 64, true); } \
      else { SAGEATTN_LAUNCH_FUSED_Q_FP8(BC_, HD_, 128, true); }
      if (head_dim == 16) {
